...
export function SafetyEquipment() {
    ...
    async function fetchProducts() {
      try {
        ...
        if (!data || data.length === 0) {
          console.log('No safety products found');
          setUseStaticData(true);
          return;
        }
  ...
  const staticSafetyProducts = staticProducts
    .filter(product => product.category === "Segurança")
    .slice(0, 6);

  const displayProducts = useStaticData ? staticSafetyProducts : products;
