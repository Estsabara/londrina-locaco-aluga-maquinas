...
import { Forklift, AlertTriangle } from "lucide-react";
...
export function MovementEquipment() {
    ...
    async function fetchProducts() {
      try {
        ...
        if (!data || data.length === 0) {
          console.log('No movement products found');
          setUseStaticData(true);
          return;
        }
  ...
  const staticMovementProducts = staticProducts
    .filter(product => product.category === "Movimentação e Elevação")
    .slice(0, 6);

  const displayProducts = useStaticData ? staticMovementProducts : products;
